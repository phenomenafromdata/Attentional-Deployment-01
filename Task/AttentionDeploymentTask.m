function AttentionDeploymentTask
%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
settings_AttentionDeployment; %load up settings
...
for thisBlock=1:n_blocks   %#ok<*NODEF>
    flag_out=false;
    %trial type sequence (first number in each entry of Pics_Seq indicates
    % image type, 1 to 5)
    ...
    for thisTrial=1:n_trials
        ...
        % select image set to present in this trial
        ...
        % get image indices within directory
        thisImageBlock=rem(Pics_Seq(:,thisTrial,thisBlock),100);
        
        % present sequence of grey-tones screens
        ...
        for img=1:size(thisImageBlock,1)
            ...
            imageTexture=Screen('MakeTexture',winPointer,imread(d(img_number).name)); % no position info provided so it'll default to position center of img at screen's center
%             Screen('DrawTexture',winPointer,imageTexture,[],facePos);
            Screen('DrawTexture',winPointer,imageTexture);
            if img==1
